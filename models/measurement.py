...
class Measurement(db.Model):
    ...
    def getDataFrom(station):
        ...
        station = station #+ ':0'
        prepare = Measurement.query.filter_by(station=station).order_by(Measurement.mtime.asc())
        for item in prepare:
    ...
    def getAllDataFromWhere(fromTime, toTime, defType = 'air_temperature'):
        ...
        for mes in returnThis:
            ...
            fixThis.append({'station': mes.station_id , 'value' : mes.value, 'time': datetime.strftime(mes.mtime, '%Y-%m-%d %H:%M')})
